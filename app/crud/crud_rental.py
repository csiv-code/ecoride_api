...
def create_rental(db: Session, rental_data: RentalCreate):
    # 1. Validación de Negocio: Verificar existencia y estado del usuario
    ...
    # 2. Validación de Negocio: Verificar existencia y disponibilidad de la bicicleta
    ...
    # 3. Marcar la bici como no disponible
...
# Búsqueda con Filtros (por user_id o status)
...
def complete_rental(db: Session, rental_id: int):
    ...
    # Finalizar y calcular costo automáticamente
    ...
    if bike:
        ...
        # Cálculo de tarifa por hora completada o fracción mínima
        duration_hours = max((end_time - rental.start_time).total_seconds() / 3600.0, 0.5)
...
def delete_rental(db: Session, rental_id: int):
    rental = get_rental(db, rental_id)
    # Si se elimina un alquiler activo, liberamos la bici por seguridad
    if rental.status == "active":
        bike = db.query(Bike).filter(Bike.id == rental.bike_id).first()
        if bike:
            bike.is_available = True
